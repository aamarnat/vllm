...
def iris_all_gather_gemm_k_shard_fake(
    ...
    _FAKE_CALL_COUNT += 1
    print(f"[META] iris_all_gather_gemm_k_shard_fake called (count: {_FAKE_CALL_COUNT}, device: {x.device})")
...
def verify_execution_counts(expected_wrapper_min=0, expected_fake_min=0):
    ...
    print(f"  iris_all_gather_gemm_k_shard_fake (meta):      {_FAKE_CALL_COUNT} calls")
    print("="*70)
...
class TestAGMMKShardModel(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        Sharding on K dimension: each rank has (M, K_local), gather to (M, K).
        
        For K-sharding:
        - Input after gather: (M, K)
        - Weight: (K, N) - no transpose needed!
        - Result: (M, K) @ (K, N) = (M, N)
        """
        # Input is already in the correct shape (M, K_local) for K-sharding
        # No reshaping needed since K is sharded, not M
        # All-gather on K dimension to get (M, K)
        all_gather = tensor_model_parallel_all_gather(hidden_states, dim=1)  # Gather on K dimension
        # For K-sharding, weight is (K, N) so no transpose needed
        mm = torch.mm(all_gather, self.weight)
        return mm
...
def simple_test():
    ...
    model = TestAGMMKShardModel(hidden_size=hidden_size, dtype=dtype)
...
def async_tp_test(local_rank, world_size, dynamic=False):
    ...
    env_vars = {
        ...
        "MASTER_PORT": "12347",  # Different port to avoid conflicts
    }
    ...
    hidden_size_total = hidden_size_per_rank * world_size  # K
    batch_size = 8
    seq_len = 16
    
    # Create model with total hidden size (weight needs to be full K dimension)
    model = TestAGMMKShardModel(hidden_size=hidden_size_total, dtype=dtype)
    ...
    # Shape: (M, K_local) where K is sharded
    M = batch_size * seq_len
    hidden_states = torch.randn(
        (M, hidden_size_per_rank),
        dtype=dtype,
        requires_grad=False
    ...
    with torch.no_grad():
        # For K-sharding, we need to gather along dim=1
        # dist.all_gather_into_tensor only supports dim=0, so use all_gather with list
        if dist.is_initialized():
            tensor_list = [torch.zeros_like(hidden_states) for _ in range(world_size)]
            dist.all_gather(tensor_list, hidden_states)
            # Concatenate along dim=1 (K dimension)
            gathered_ref = torch.cat(tensor_list, dim=1)
        else:
        ...
        # For K-sharding, weight is already in (K, N) format, no transpose needed
