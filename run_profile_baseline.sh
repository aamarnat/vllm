#!/usr/bin/env bash
# Baseline profiling: standard vLLM without fused GEMM+AllReduce.
# No fuse_gemm_all_reduce, no SP, no async-TP — plain tensor parallelism.
set -euo pipefail
...
export RESULT_FILENAME="gptoss_baseline_results"
export PORT=8088
...
export VLLM_TORCH_PROFILER_DIR=./vllm_profile/baseline/
mkdir -p ./vllm_profile/baseline/

SERVER_LOG=$(mktemp /tmp/server-baseline-XXXXXX.log)
...
/opt/venv/bin/python -m vllm.entrypoints.cli.main serve "$MODEL" \
    ...
    --profiler-config '{
        "profiler": "torch",
        "torch_profiler_dir": "./vllm_profile/baseline",
